...
protected:
    ...
    typedef CAtlMap<DWORD, HFONT> _TypeFontPool;
...
public:
    ...
    static HFONT GetFont(WORD uKey, BOOL bDpiAware = FALSE)
    {
        DWORD dwKey = uKey | bDpiAware << 31;
        _TypeFontPool::CPair* pPairRet = _Instance()->m_mapFont.Lookup(dwKey);
        HFONT hftRet = NULL;

        if (NULL == pPairRet)
        {
            hftRet = _Instance()->_CreateNewFont(
                BKF_ISBOLD(uKey), BKF_ISUNDERLINE(uKey), BKF_ISITALIC(uKey), BKF_GETADDING(uKey), bDpiAware
                );
            if (hftRet)
                _Instance()->m_mapFont[dwKey] = hftRet;
        }
        else
    ...
    static HFONT GetFont(BOOL bBold, BOOL bUnderline, BOOL bItalic, char chAdding = 0, BOOL bDpiAware = FALSE)
    {
        return GetFont(BKF_MAKEKEY(bBold, bUnderline, bItalic, chAdding), bDpiAware);
    }
	...
	static int GetFontSizeAdding(HFONT hft, BOOL bDpiAware = FALSE)
	{
		LOGFONT lf;
		::GetObject(hft, sizeof(LOGFONT), &lf);

        LONG lDpiSize = _Instance()->m_lfDefault.lfHeight * BKWinDpiAware::GetInstance().GetDpiScaleRate();
        return (bDpiAware ? lDpiSize : _Instance()->m_lfDefault.lfHeight) - lf.lfHeight;
	}
...
protected:
    ...
    HFONT _CreateNewFont(BOOL bBold, BOOL bUnderline, BOOL bItalic, char chAdding, BOOL bDpiAware = FALSE)
    {
        LOGFONT lfNew;
        ...
        lfNew.lfHeight = _GetFontAbsHeight(lfNew.lfHeight - chAdding, bDpiAware);
    ...
    inline LONG _GetFontAbsHeight(LONG lSize, BOOL bDpiAware = FALSE)
    {
        int iDpiSize = 0;
        iDpiSize = lSize * BKWinDpiAware::GetInstance().GetDpiScaleRate();
        return bDpiAware ? iDpiSize : lSize;
    }
};
