...
public:
	...
	CRichTextGdiGraphics(HDC hDC, const BkStyle& bkStyle, DWORD dwObjState, BOOL bSetSupportDpiAware)
		: m_bkStyle(bkStyle)
		, m_dcHanle(hDC)
        ...
        , m_bSupportDpiAware(bSetSupportDpiAware)
	{
		InitImpl();
	}
...
protected:
	...
	void InitImpl()
	{
		...
		HFONT	  hFont = BkFontPool::GetFont(FALSE, FALSE, FALSE, 0, m_bSupportDpiAware);
		...
		else if ((m_dwState & BkWndState_PushDown || m_dwState & BkWndState_Hover))
		{
			...
			if (m_bkStyle.m_wFtHover)
                hFont = BkFontPool::GetFont(m_bkStyle.m_wFtHover, m_bSupportDpiAware);
		...
		else if ((m_dwState & BkWndState_Normal) == BkWndState_Normal)
		{
			...
			if (m_bkStyle.m_wFtText)
                hFont = BkFontPool::GetFont(m_bkStyle.m_wFtText, m_bSupportDpiAware);
...
public:
	...
	CRichTextGdiPlusGraphics(HDC hDC, const BkStyle& bkStyle, DWORD dwObjState, BOOL bSetSupportDpiAware)
		: m_bkStyle(bkStyle)
		, m_dcHanle(hDC)
		, m_dwState (dwObjState)
		, m_pGraphics(NULL)
...
protected:
	...
	virtual IRichTextGraphics* GetGraphics(HDC hDC)
	{
		...
		{
			pIRichTextGraphics = new CRichTextGdiPlusGraphics(hDC, bkStyle, GetState(), m_bSupportDpiAware);
		}
		else
		{
			pIRichTextGraphics = new CRichTextGdiGraphics(hDC, bkStyle, GetState(), m_bSupportDpiAware);
		}
...
public:
	...
	virtual void   MeasurePositon(HDC hDC, CPointF& ptNext, CRectF& rcLayout)
	{
		...
		for (int nIndex = 0; nIndex < nTextCount;)
		{
			...
			if (pCharObj)
			{
				rcDraw.OffsetRect(0, m_bSupportDpiAware ? BKWinDpiAware::GetInstance().ScaleByDpi(m_nAdjust) : m_nAdjust);
	...
	virtual BOOL IsNeedRedrawWhenStateChange()
	{
		...
		if ((bkStyle.m_crText != bkStyle.m_crHoverText && bkStyle.m_crHoverText != CLR_INVALID && bkStyle.m_crText != CLR_INVALID)
			|| (bkStyle.m_wFtHover != bkStyle.m_wFtText && bkStyle.m_wFtHover != 0 && bkStyle.m_wFtText != 0) 
