...
typedef struct
{
	BOOL bItemCheckState; // 如果是checkbox，对应的值。
	DWORD dwItemCheckBoxUniqueId;

}BKLISTEMCHILDNOFITY_CHECKBOX, *PBKLISTEMCHILDNOFITY_CHECKBOX;
...
typedef struct
{
	...
	BKLISTEMCHILDNOFITY_CHECKBOX checkbox;
...
#define BK_LISTWND_LISTITEM_CHILD_LBUTTONUP_CHECKBOX(func)\
	if(uListCode == BKNM_LISTWND_LISTITEM_CHILD_LBUTTONUP){\
	func(	((PBKNOTIFYLISTITEM)pnmh)->nItem, \
			((PBKNOTIFYLISTITEM)pnmh)->childNotify.uCmdID, \
			((PBKNOTIFYLISTITEM)pnmh)->childNotify.szItemClass, \
			((PBKNOTIFYLISTITEM)pnmh)->childNotify.checkbox );\
	}
