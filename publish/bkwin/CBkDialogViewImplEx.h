...
public:
	...
	CBkWindow* CreateBkControlByName( LPCSTR lpszName )
	{
		...
		pWnd = BkWin::BkLinearLayout::CheckAndNew(lpszName);
		if (pWnd) 
		{
			static_cast<CBkPanel*>(pWnd)->SetExternCreator(this);
			return pWnd;
...
public:
	CBkDialogViewImplEx(UINT uResID = 0) : m_uResID(uResID)
	{
		...
		m_bReadyedCreateWnd = FALSE;
		m_bShouldRepositonViews = FALSE;
...
protected:
	...
	BOOL m_bReadyedCreateWnd;
	BOOL m_bShouldRepositonViews;
...
public:
	...
	HWND CreateEx(HWND hWndParent = ::GetActiveWindow(), UINT uCtrlID = 0, LPRECT rect = NULL)
	{
		...
		m_bShowWindow = TRUE;
		m_bReadyedCreateWnd = TRUE;
		SendMessage(WM_INITDIALOG, (WPARAM)hWnd);
		OnBkDialogViewCreate();

		m_bShouldRepositonViews = TRUE;
		MoveWindow(rcWnd, FALSE);
		if(m_bShouldRepositonViews) {
				RecomposeItems(TRUE);
			}

		ShowAllRealWindows(TRUE);
		OnBkDialogViewCreateFinish();
...
protected:
	...
	virtual void OnBkDialogViewCreate() {
	}
	virtual void OnBkDialogViewCreateFinish() {
	}
