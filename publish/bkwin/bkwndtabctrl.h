...
//         }
    ...
    CRect ScaleByRect(const CRect& rc)
    {
        CRect destRc;
        destRc.left = ScaleByDpi(rc.left);
        destRc.right = ScaleByDpi(rc.right);
        destRc.top = ScaleByDpi(rc.top);
        destRc.bottom = ScaleByDpi(rc.bottom);
        return destRc;
    }

    CRect ScaleIcon(const CRect& SrcRect, const SIZE& sizeIcon)
    {
       SIZE new_sizeicon;
       new_sizeicon.cx = ScaleByDpi(sizeIcon.cx);
       new_sizeicon.cy = ScaleByDpi(sizeIcon.cy);

       CRect new_Rect = SrcRect;
       new_Rect.right = new_Rect.left + new_sizeicon.cx;
       new_Rect.bottom = SrcRect.top + new_sizeicon.cy;
       return new_Rect;
    }
    ...
    void OnPaint(CDCHandle dc)
    {
        ...
        if (pSkinFrame)
            pSkinFrame->Stretch(dc, rcItem, BkWndState_Normal, IsDpiAwareEnable());
        ...
        for (int i = 0; i < nPageCount; i ++)
        {
            ...
            if (pSkinSplitter && nVisibleCount > 1)
            {
                ...
                pSkinSplitter->Stretch(dc, rcDraw, (DWORD)-1, IsDpiAwareEnable()); 
            }
			...
			if (m_pPaintCallBack)
			{
				...
				{
					if (pSkinTab)
                        pSkinTab->Stretch(dc, rcItem, (i != m_nHoverTabItem) ? BkWndState_Normal : BkWndState_Hover, IsDpiAwareEnable()); 
				}
			}
			else
			{
            ...
            if (pSkinIcon)
            {
                ...
                pSkinIcon->Stretch(dc, ScaleIcon(rcDraw, sizeIcon), i, IsDpiAwareEnable());
            }
        ...
        for (int i = 0; i < nPageCount; i ++)
        {
			...
			if (m_pPaintCallBack)
			{
				...
				{
					if (pSkinTab)
                        pSkinTab->Stretch(dc, rcItem, BkWndState_PushDown, IsDpiAwareEnable());
				}
			}
			else
			{
        ...
        if (m_nVertFont != 0)
        {
            ...
            for (int i = 0; i < nLen; i++)
            {
				...
				if (BkFontPool::IsYaHei())
					rc.bottom = rc.top + BkFontPool::GetDefaultFontSize() + BkFontPool::GetFontSizeAdding(dc.GetCurrentFont(), IsDpiAwareEnable());
    ...
    HRESULT OnTabWidth(CStringA& strValue, BOOL bLoading)
    { 
        int nValue = ::StrToIntA(strValue);
        m_nTabWidth = ScaleByDpi(nValue);
        return 0;
    }

    HRESULT OnTabHeight(CStringA& strValue, BOOL bLoading)
    { 
        int nValue = ::StrToIntA(strValue);
        m_nTabHeight = ScaleByDpi(nValue);
        return 0;
    }

    HRESULT OnIconX(CStringA& strValue, BOOL bLoading)
    { 
        int nValue = ::StrToIntA(strValue);
        m_ptIcon.x = ScaleByDpi(nValue);
        return 0;
    }

    HRESULT OnIconY(CStringA& strValue, BOOL bLoading)
    { 
        int nValue = ::StrToIntA(strValue);
        m_ptIcon.y = ScaleByDpi(nValue);
        return 0;
    }

    HRESULT OnTextX(CStringA& strValue, BOOL bLoading)
    { 
        int nValue = ::StrToIntA(strValue);
        m_ptText.x = ScaleByDpi(nValue);
        return 0;
    }

    HRESULT OnTextY(CStringA& strValue, BOOL bLoading)
    { 
        int nValue = ::StrToIntA(strValue);
        m_ptText.y = ScaleByDpi(nValue);
        return 0;
    }
...
protected:
    ...
    BKWIN_DECLARE_ATTRIBUTES_BEGIN()
        ...
        BKWIN_CUSTOM_ATTRIBUTE("tabwidth", OnTabWidth)
        BKWIN_CUSTOM_ATTRIBUTE("tabheight", OnTabHeight)
...
//         BKWIN_SKIN_ATTRIBUTE("splitterskin", m_pSkinSplitter, FALSE)
        ...
        BKWIN_CUSTOM_ATTRIBUTE("icon-x", OnIconX)
        BKWIN_CUSTOM_ATTRIBUTE("icon-y", OnIconY)
        //BKWIN_INT_ATTRIBUTE("text-x", m_ptText.x, FALSE)
        //BKWIN_INT_ATTRIBUTE("text-y", m_ptText.y, FALSE)
        BKWIN_CUSTOM_ATTRIBUTE("text-x", OnTextX)
        BKWIN_CUSTOM_ATTRIBUTE("text-y", OnTextY)
