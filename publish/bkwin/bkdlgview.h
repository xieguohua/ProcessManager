...
{
public:
    CBkViewImpl() : m_bSupportDpiAware(FALSE)
    {
	...
	{
		return IsDpiAwareEnable() ? lSize / BKWinDpiAware::GetInstance().GetDpiScaleRate() : lSize;
	}
...
public:
    ...
    void OnPrint2(CDCHandle dc, UINT uFlags)
    {
        ...
        if (m_bNeedRepaint)
        {
            ...
            HFONT hftOld = dc.SelectFont(BkFontPool::GetFont(BKF_DEFAULTFONT, IsDpiAwareEnable()));
...
protected:
    ...
    void OnPrint(CDCHandle dc, UINT uFlags)
    {
        ...
        if (m_bNeedRepaint)
        {
            ...
            HFONT hftOld = dcMem.SelectFont(BkFontPool::GetFont(BKF_DEFAULTFONT, IsDpiAwareEnable()));
...
public:
    ...
    long ScaleByDpi(long lSize)
    {
        return m_richView.ScaleByDpi(lSize);
    }

    BOOL IsDpiAwareEnable()
    {
        return  m_richView.IsDpiAwareEnable();
    }
...
public:
    ...
    HWND Create(HWND hWndParent = ::GetActiveWindow(), LPRECT rect = NULL, _U_MENUorID MenuOrID = 0U)
    {
        ...
        BKEnableThreadAwarenessContextWarp threadDpiAwareWarp(m_richView.m_bSupportDpiAware);
