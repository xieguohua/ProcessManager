...
// 此宏定义是否默认开启：正确DrawBkgnd class skin
	...
	const BkWin::BkLayoutParam& GetLayoutParam() const {
		return m_layoutParam;
	}

	BkWin::BkLayoutParam& GetLayoutParam() {
		return m_layoutParam;
	}
...
protected:
	...
	BkWin::BkLayoutParam m_layoutParam;
public:
   ...
   virtual BOOL IsHeadAcceptLBtnDownMsg()
   {
    ...
    HRESULT SetInnerText(LPCTSTR lpszText)
    {
        ...
        if ((m_uPositionType & SizeX_FitContent | SizeY_FitContent) || (CLR_INVALID == m_crBg && CLR_INVALID == GetStyle().m_crBg))
        {
            if ((m_uPositionType & (SizeX_FitContent | SizeY_FitContent)) && (4 != m_dlgpos.nCount))
            {
				...
				_NotifyAttributeChangeToParent("text");
            }
    ...
    void SetVisible(BOOL bVisible)
    {
		...
		_NotifyAttributeChangeToParent("show");
    }
...
public:
    ...
    {
		if(lpWndPos->flags & SWP_NOREPOSITION == SWP_NOREPOSITION) {
			//表示不需要重新计算位置：直接设置位置
			 m_rcWindow.SetRect(lpWndPos->x, lpWndPos->y,
    ...
    LRESULT OnNcCalcSize(BOOL bCalcValidRects, LPARAM lParam)
    {
		...
		{	
			if(m_layoutParam.GetLayoutWidth() !=  BkWin::WRAP_CONTENT &&
				m_layoutParam.GetLayoutHeight()  != BkWin::WRAP_CONTENT) 
			{
				if(m_layoutParam.GetLayoutWidth() >=0 ) {
					pSize->cx = m_layoutParam.GetLayoutWidth();
				}

				if(m_layoutParam.GetLayoutHeight() >=0 ) {
					pSize->cy = m_layoutParam.GetLayoutHeight();
				}
				return 0;
			}
			...
			if (0 != m_strInnerText.GetLength())
			{
				if (theStyle.m_bGdiplusCalcTxt)
				{
					...
					if (m_layoutParam.GetLayoutWidth() >= 0) {
						layoutSize.Width = m_layoutParam.GetLayoutWidth();
					}
					else if(m_layoutParam.GetLayoutMaxWidth() == BkWin::MATCH_PARENT) {
						layoutSize.Width = sizeParent.cx;
					}

					if (m_layoutParam.GetLayoutHeight() >= 0) {
						layoutSize.Height = m_layoutParam.GetLayoutHeight();
					}
					else if(m_layoutParam.GetLayoutMaxWidth() == BkWin::MATCH_PARENT) {
						layoutSize.Height = sizeParent.cy;
					}

					layoutSize.Width = BKWIN_BKLAYOUTPARAM_MAX(layoutSize.Width, m_layoutParam.GetLayoutMinWidth());
					layoutSize.Height = BKWIN_BKLAYOUTPARAM_MAX(layoutSize.Height, m_layoutParam.GetLayoutMinHeight());
					layoutSize.Width = BKWIN_BKLAYOUTPARAM_MIN(layoutSize.Width, m_layoutParam.GetLayoutMaxWidth());
					layoutSize.Height = BKWIN_BKLAYOUTPARAM_MIN(layoutSize.Height, m_layoutParam.GetLayoutMaxHeight());
				...
				{
					if (m_layoutParam.GetLayoutWidth() >= 0) {
						rcTest.right = rcTest.left + m_layoutParam.GetLayoutWidth();
					}
					else if(m_layoutParam.GetLayoutMaxWidth() == BkWin::MATCH_PARENT) {
						rcTest.right = rcTest.left + sizeParent.cx;
					}

					if (m_layoutParam.GetLayoutHeight() >= 0) {
						rcTest.bottom = rcTest.top + m_layoutParam.GetLayoutHeight();
					}
					else if(m_layoutParam.GetLayoutHeight() == BkWin::MATCH_PARENT) {
						rcTest.bottom = rcTest.top + sizeParent.cy;
					}

					rcTest.right = rcTest.left + BKWIN_BKLAYOUTPARAM_MAX(rcTest.Width(), m_layoutParam.GetLayoutMinWidth());
					rcTest.bottom = rcTest.top + BKWIN_BKLAYOUTPARAM_MAX(rcTest.Height(), m_layoutParam.GetLayoutMinHeight());

					rcTest.right = rcTest.left + BKWIN_BKLAYOUTPARAM_MIN(rcTest.Width(), m_layoutParam.GetLayoutMaxWidth());
					rcTest.bottom = rcTest.top + BKWIN_BKLAYOUTPARAM_MIN(rcTest.Height(), m_layoutParam.GetLayoutMaxHeight());
			...
			if (m_layoutParam.GetLayoutHeight() >= 0) {
				...
				pSize->cy = sizeParent.cy;
			}

			 pSize->cx = BKWIN_BKLAYOUTPARAM_MAX( pSize->cx, m_layoutParam.GetLayoutMinWidth());
			 pSize->cy = BKWIN_BKLAYOUTPARAM_MAX(pSize->cy, m_layoutParam.GetLayoutMinHeight());
			 pSize->cx = BKWIN_BKLAYOUTPARAM_MIN(pSize->cx, m_layoutParam.GetLayoutMaxWidth());
			 pSize->cy = BKWIN_BKLAYOUTPARAM_MIN(pSize->cy, m_layoutParam.GetLayoutMaxHeight());
	...
	void _NotifyAttributeChangeToParent(LPCSTR lpszAttrName);
...
//     };
    BKWIN_DECLARE_ATTRIBUTES_BEGIN()
		...
		BKWIN_CHAIN_ATTRIBUTE(m_layoutParam, FALSE)
...
inline void CBkWindow::_NotifyAttributeChangeToParent(LPCSTR lpszAttrName)
{
	if(GetParent() == NULL) {
		return;
	}
	...
	{
		CBkContainerWnd *pWndParentContainer = (CBkContainerWnd *)pWndParent;
		pWndParentContainer->OnChildAttributeChanged(this, lpszAttrName);
	}
}
